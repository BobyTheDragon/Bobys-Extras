...
public class ToolGlassSword extends ItemSword {
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
    	par3List.add("Very Fragile... And Deadly");
    ...
    public boolean onDroppedByPlayer(ItemStack itemstack, EntityPlayer player) {
		if (player.isCreative() == false) {
    	    --itemstack.stackSize;
			player.addChatComponentMessage(new TextComponentString("\u00A7o\u00A7eIt shattered... What did you think would happen when you dropped it? Its made of glass!"));
		}
        return true;
    }
			...
			if (Player.getHeldItemMainhand() != null && Player.getHeldItemMainhand().getItem() instanceof ToolGlassSword && Player.isCreative() == false) {
				...
				if (Player.isBurning()) {
					if (BreakChance == 1) {
			            ...
			            Player.worldObj.playSound(Player.posX, Player.posY, Player.posZ, SoundEvents.BLOCK_GLASS_BREAK, SoundCategory.PLAYERS, 1f, 1f, false);
			            Player.playSound(SoundEvents.BLOCK_GLASS_BREAK, 1f, 1f);
						Player.addChatComponentMessage(new TextComponentString("\u00A7o\u00A7eOh... The Glass Sword melted in the heat of the flames..."));
					}
				}
					...
					if (BreakChance == 2) {
			            ...
			            Player.worldObj.playSound(Player.posX, Player.posY, Player.posZ, SoundEvents.BLOCK_GLASS_BREAK, SoundCategory.PLAYERS, 1f, 1f, false);
			            Player.playSound(SoundEvents.BLOCK_GLASS_BREAK, 1f, 1f);
						Player.addChatComponentMessage(new TextComponentString("\u00A7o\u00A7eThe Glass Sword fell out of your hand and smashed on the ground, Next time try not to run..."));
					}
					else if (BreakChance == 3) {
						Player.performHurtAnimation();
						Player.setHealth(Player.getHealth() - 1);
						Player.addChatComponentMessage(new TextComponentString("\u00A7o\u00A7eOuch! Dont run! You cut yourself!"));
					}
				}
			}
    	}
    ...
    public boolean hitEntity(ItemStack itemstack, EntityLivingBase entity1, EntityLivingBase entity2) {
			...
			if (Player.isCreative() == false) {
    	    --itemstack.stackSize;
			Player.addChatComponentMessage(new TextComponentString("\u00A7o\u00A7eThe Glass Sword smashes on contact... Ouch!"));
			}
    	    return true;
    }
}
